...
class Widget_Post extends WP_Widget {
    ...
    function widget($args, $instance):void
    {
        ...
        //var_dump((int)$instance['category']);
        if($instance['category']!=''&& $instance['category'] != 0){
            $args=array('cat'=>(int)$instance['category']);
        }else{
            $args=array();//toutes les catégories
        }
...
function my_shortcode($attributes,$content,$shortcodeTag){
    $html='<p>Bonjour '.($attributes['name']??'').' depuis le shortcode '.$shortcodeTag.'.<br> '.$content.'</p>';
    return $html;
}
